...
const navGroupsDef: { titleKey: string; items: NavItem[] }[] = [
  { titleKey: 'nav.foundations', items: [
    { id: 'accueil', labelKey: 'nav.accueil', num: '00', icon: <Home className="w-4 h-4" /> },
  ...
  return (
    <div className="space-y-6">
      ...
      <header>
        ...
        <p className="text-navy-400 mt-2 ml-[52px]">{t('header.subtitle')}</p>
      </header>
      ...
      <button onClick={() => setMobileNavOpen(!mobileNavOpen)} className="lg:hidden w-full flex items-center justify-between px-4 py-3 rounded-xl bg-navy-50 text-navy-700 font-medium text-sm">
        <span>{t('nav.navigation')} · {t(navGroupsDef.flatMap(g => g.items).find(i => i.id === activeSection)?.labelKey ?? '')}</span>
          ...
          <div className="lg:sticky lg:top-24 space-y-4">
            {navGroupsDef.map((group) => (
              <div key={group.titleKey}>
                <p className="text-[10px] font-semibold tracking-wider uppercase text-navy-300 px-3 mb-1.5">{t(group.titleKey)}</p>
...
function AccueilPanel({ onNavigate }: { onNavigate: (s: Section) => void }) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        <p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('accueil.program')}</p>
        <h2 className="text-xl font-bold text-navy-900 mb-2">{t('accueil.title')}</h2>
        <p className="text-sm text-navy-400 leading-relaxed max-w-2xl">{t('accueil.subtitle')}</p>
      </div>
      {/* Exam structure */}
      <div className="rounded-xl border border-cream-200 bg-white p-5">
        <h3 className="text-base font-semibold text-navy-900 mb-4 flex items-center gap-2"><BarChart3 className="w-4 h-4 text-blue-600" />{t('accueil.examStructure')}</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm"><thead><tr className="border-b border-cream-200"><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('accueil.section')}</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('accueil.duration')}</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('accueil.format')}</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('accueil.scale')}</th></tr></thead>
          <tbody className="divide-y divide-cream-100">
            <tr><td className="py-2 px-3 font-medium">CO</td><td className="py-2 px-3 text-navy-500">{t('accueil.co_duration')}</td><td className="py-2 px-3 text-navy-500">{t('accueil.co_format')}</td><td className="py-2 px-3 font-mono">/360</td></tr>
            <tr><td className="py-2 px-3 font-medium">CE</td><td className="py-2 px-3 text-navy-500">{t('accueil.ce_duration')}</td><td className="py-2 px-3 text-navy-500">{t('accueil.ce_format')}</td><td className="py-2 px-3 font-mono">/300</td></tr>
            <tr><td className="py-2 px-3 font-medium">EE</td><td className="py-2 px-3 text-navy-500">{t('accueil.ee_duration')}</td><td className="py-2 px-3 text-navy-500">{t('accueil.ee_format')}</td><td className="py-2 px-3 font-mono">/450</td></tr>
            <tr><td className="py-2 px-3 font-medium">EO</td><td className="py-2 px-3 text-navy-500">{t('accueil.eo_duration')}</td><td className="py-2 px-3 text-navy-500">{t('accueil.eo_format')}</td><td className="py-2 px-3 font-mono">/450</td></tr>
          </tbody></table>
        </div>
        <p className="text-xs text-navy-400 mt-3 italic">{t('accueil.nclcNote')}</p>
      </div>
      {/* Quick access */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {([
          { s: 'diagnostic' as Section, icon: <Target className="w-5 h-5" />, label: t('accueil.diagnosticLabel'), sub: t('accueil.diagnosticSub'), from: 'from-blue-500', to: 'to-indigo-600' },
          { s: 'pieges' as Section, icon: <AlertTriangle className="w-5 h-5" />, label: t('accueil.piegesLabel'), sub: t('accueil.piegesSub'), from: 'from-amber-500', to: 'to-orange-600' },
          { s: 'ce' as Section, icon: <FileText className="w-5 h-5" />, label: t('accueil.ceLabel'), sub: t('accueil.ceSub'), from: 'from-red-500', to: 'to-pink-600' },
          { s: 'co' as Section, icon: <Headphones className="w-5 h-5" />, label: t('accueil.coLabel'), sub: t('accueil.coSub'), from: 'from-sky-500', to: 'to-blue-600' },
...
function ReferentielPanel() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('ref.foundation')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('ref.title')}</h2><p className="text-sm text-navy-400 leading-relaxed max-w-2xl">{t('ref.subtitle')}</p></div>
      <div className="rounded-xl border border-cream-200 bg-white p-5"><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b-2 border-navy-100"><th className="text-left py-2.5 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('ref.stage')}</th><th className="text-left py-2.5 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('ref.levels')}</th><th className="text-left py-2.5 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('ref.cecr')}</th><th className="text-left py-2.5 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{t('ref.profile')}</th></tr></thead><tbody className="divide-y divide-cream-100"><tr><td className="py-2.5 px-3 font-medium">{t('ref.stageI')}</td><td className="py-2.5 px-3">1 à 4</td><td className="py-2.5 px-3 font-mono text-xs">A1 → A2</td><td className="py-2.5 px-3 text-navy-500">{t('ref.profileI')}</td></tr><tr className="bg-blue-50/50"><td className="py-2.5 px-3 font-medium">{t('ref.stageII')}</td><td className="py-2.5 px-3">5 à 8</td><td className="py-2.5 px-3 font-mono text-xs">A2/B1 → B2</td><td className="py-2.5 px-3 text-navy-500">{t('ref.profileII')}</td></tr><tr><td className="py-2.5 px-3 font-medium">{t('ref.stageIII')}</td><td className="py-2.5 px-3">9 à 12</td><td className="py-2.5 px-3 font-mono text-xs">C1 → C2</td><td className="py-2.5 px-3 text-navy-500">{t('ref.profileIII')}</td></tr></tbody></table></div></div>
      <div className="rounded-xl border border-cream-200 bg-white p-5"><h3 className="text-base font-semibold text-navy-900 mb-4">{t('ref.descriptors')} <span className="text-xs text-navy-400 font-normal">{t('ref.priorityZone')}</span></h3><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b-2 border-navy-100"><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">NCLC</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">CE</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">CO</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">EE</th><th className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">EO</th></tr></thead><tbody className="divide-y divide-cream-100"><tr><td className="py-2 px-3 font-mono font-bold">5</td><td className="py-2 px-3 text-navy-500">{t('ref.ce5')}</td><td className="py-2 px-3 text-navy-500">{t('ref.co5')}</td><td className="py-2 px-3 text-navy-500">{t('ref.ee5')}</td><td className="py-2 px-3 text-navy-500">{t('ref.eo5')}</td></tr><tr><td className="py-2 px-3 font-mono font-bold">6</td><td className="py-2 px-3 text-navy-500">{t('ref.ce6')}</td><td className="py-2 px-3 text-navy-500">{t('ref.co6')}</td><td className="py-2 px-3 text-navy-500">{t('ref.ee6')}</td><td className="py-2 px-3 text-navy-500">{t('ref.eo6')}</td></tr><tr className="bg-yellow-50/50 font-medium"><td className="py-2 px-3 font-mono font-bold text-amber-700">7 ⭐</td><td className="py-2 px-3">{t('ref.ce7')}</td><td className="py-2 px-3">{t('ref.co7')}</td><td className="py-2 px-3">{t('ref.ee7')}</td><td className="py-2 px-3">{t('ref.eo7')}</td></tr><tr><td className="py-2 px-3 font-mono font-bold">8</td><td className="py-2 px-3 text-navy-500">{t('ref.ce8')}</td><td className="py-2 px-3 text-navy-500">{t('ref.co8')}</td><td className="py-2 px-3 text-navy-500">{t('ref.ee8')}</td><td className="py-2 px-3 text-navy-500">{t('ref.eo8')}</td></tr></tbody></table></div></div>
    </div>
  );
}
...
function LexiquePanel() {
    ...
    return (
      <div className="space-y-5 animate-in fade-in duration-300">
        <button onClick={() => { setSelectedDomain(null); setMode('browse'); }} className="text-sm text-navy-400 hover:text-navy-700 flex items-center gap-1">
          <ChevronDown className="w-4 h-4 rotate-90" /> {t('lex.backDomains')}
        </button>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-xs text-navy-400">{selectedDomain.entries.length} {t('lex.terms')} · {selectedDomain.quizQuestions.length} {t('lex.questions')}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {([
            { key: 'browse' as const, label: t('lex.browse'), icon: <BookOpen className="w-3.5 h-3.5" /> },
            { key: 'flashcard' as const, label: t('lex.flashcards'), icon: <Layers className="w-3.5 h-3.5" /> },
            { key: 'quiz' as const, label: t('lex.quiz'), icon: <Target className="w-3.5 h-3.5" /> },
            { key: 'dictee' as const, label: t('lex.dictee'), icon: <Headphones className="w-3.5 h-3.5" /> },
          ]).map(m => (
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div>
        <p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('lex.foundation')}</p>
        <h2 className="text-xl font-bold text-navy-900 mb-2">{t('lex.title')}</h2>
        <p className="text-sm text-navy-400 max-w-2xl">
          {t('lex.subtitle_tpl', { domains: String(LEXIQUE_STATS.totalDomains), entries: String(LEXIQUE_STATS.totalEntries), questions: String(LEXIQUE_STATS.totalQuestions) })}
        </p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: t('lex.stageI'), count: LEXIQUE_STATS.byStage[1], color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
          { label: t('lex.stageII'), count: LEXIQUE_STATS.byStage[2], color: 'bg-blue-50 text-blue-700 border-blue-200' },
          { label: t('lex.stageIII'), count: LEXIQUE_STATS.byStage[3], color: 'bg-amber-50 text-amber-700 border-amber-200' },
        ].map(s => (
        ...
        {lexiqueDomains.map(d => (
          <button key={d.id} onClick={() => setSelectedDomain(d)}
            ...
            <div className="flex items-center gap-3 text-[11px] text-navy-400">
              <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" />{d.entries.length} {t('lex.terms')}</span>
              <span className="flex items-center gap-1"><Target className="w-3 h-3" />{d.quizQuestions.length} {t('lex.quiz')}</span>
...
function LexiqueBrowse({ domain }: { domain: LexiqueDomain }) {
  ...
  const stageLabels: Record<1|2|3, string> = { 1: t('lex.stage1'), 2: t('lex.stage2'), 3: t('lex.stage3') };
      ...
      <div className="flex gap-2 flex-wrap">
        {([0, 1, 2, 3] as const).map(s => (
          <button key={s} onClick={() => setFilterStage(s)}
            ...
            {s === 0 ? `${t('lex.all')} (${domain.entries.length})` : `${stageLabels[s]} (${domain.entries.filter(e => e.stage === s).length})`}
          </button>
        ))}
      </div>
      {filtered.map((entry, idx) => (
        <div key={idx} className="rounded-xl border border-cream-200 bg-white p-4 space-y-2">
          ...
          {entry.synonyms.length > 0 && (
            <p className="text-xs text-navy-400">{t('lex.synonyms')} <span className="font-medium text-navy-600">{entry.synonyms.join(', ')}</span></p>
          )}
        </div>
      ))}
    </div>
...
function LexiqueFlashcards({ domain }: { domain: LexiqueDomain }) {
  ...
  const markAndNext = (isKnown: boolean) => {
    if (isKnown) { setKnown(p => new Set(p).add(cardIdx)); addXp(TEF_XP_CONFIG.qcm_correct, 'TEF Lexique flashcard'); }
    else { setToReview(p => new Set(p).add(cardIdx)); }
    setFlipped(false);
    if (cardIdx < total - 1) setCardIdx(cardIdx + 1);
  };

  const reshuffle = () => { setShuffled([...domain.entries].sort(() => Math.random() - 0.5)); setCardIdx(0); setFlipped(false); setKnown(new Set()); setToReview(new Set()); };
  ...
  if (!card || allDone) {
    return (
      <div className="rounded-xl border border-cream-200 bg-white p-6 text-center space-y-4">
        ...
        <h3 className="text-lg font-bold text-navy-900">{t('fc.sessionDone')}</h3>
        <div className="flex justify-center gap-6 text-sm">
          <span className="text-emerald-600 font-bold"><ThumbsUp className="w-4 h-4 inline mr-1" />{known.size} {t('fc.mastered')}</span>
          <span className="text-amber-600 font-bold"><ThumbsDown className="w-4 h-4 inline mr-1" />{toReview.size} {t('fc.toReview')}</span>
        </div>
        <p className="text-xs text-navy-400">{t('fc.xpEarned', { xp: known.size * TEF_XP_CONFIG.qcm_correct })}</p>
        <button onClick={reshuffle} className="px-5 py-2.5 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 inline-flex items-center gap-2">
          <Shuffle className="w-4 h-4" />{t('fc.restart')}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 text-xs text-navy-400">
        ...
        <div className="flex-1 bg-cream-100 rounded-full h-2 overflow-hidden"><div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${progress}%` }} /></div>
        <span className="text-emerald-600 font-bold">{known.size} ✓</span>
        <span className="text-amber-600 font-bold">{toReview.size} ↻</span>
      </div>
      <div onClick={() => setFlipped(!flipped)} className="rounded-2xl border-2 border-cream-200 bg-white min-h-[250px] p-6 flex flex-col items-center justify-center cursor-pointer hover:shadow-lg transition-shadow relative">
        {!flipped ? (
          <div className="text-center space-y-4">
            ...
            <button onClick={(e) => { e.stopPropagation(); tts.speak(card.term, `fc-${cardIdx}`, 0.85); }}
              ...
              <Volume2 className="w-3.5 h-3.5" />{t('lex.listen')}
            </button>
            <p className="text-xs text-navy-300 mt-4">{t('fc.tapToFlip')}</p>
          </div>
        ) : (
      ...
      {flipped && (
        <div className="flex gap-3">
          <button onClick={() => markAndNext(false)} className="flex-1 py-3 rounded-xl bg-amber-50 text-amber-700 font-semibold text-sm border border-amber-200 hover:bg-amber-100 flex items-center justify-center gap-2">
            <RotateCcw className="w-4 h-4" />{t('fc.reviewBtn')}
          </button>
          <button onClick={() => markAndNext(true)} className="flex-1 py-3 rounded-xl bg-emerald-600 text-white font-semibold text-sm hover:bg-emerald-500 flex items-center justify-center gap-2">
            <CheckCircle2 className="w-4 h-4" />{t('fc.iKnow')}
          </button>
        </div>
      )}
    </div>
...
function LexiqueQuiz({ domain }: { domain: LexiqueDomain }) {
  ...
  useEffect(() => { const shuffled = [...domain.quizQuestions].sort(() => Math.random() - 0.5).slice(0, QUIZ_SIZE); setQuestions(shuffled); setCurrentIdx(0); setSelected(null); setScore(0); setAnswered(0); setShowResult(false); }, [domain]);

  const q = questions[currentIdx];
  const isFinished = answered >= questions.length && questions.length > 0;

  const handleSelect = (idx: number) => { if (selected !== null) return; setSelected(idx); if (idx === q.correctIndex) { setScore(s => s + 1); addXp(TEF_XP_CONFIG.qcm_correct, 'TEF Lexique quiz'); } setAnswered(a => a + 1); };
  const nextQuestion = () => { if (currentIdx < questions.length - 1) { setCurrentIdx(currentIdx + 1); setSelected(null); } else setShowResult(true); };
  const restart = () => { const shuffled = [...domain.quizQuestions].sort(() => Math.random() - 0.5).slice(0, QUIZ_SIZE); setQuestions(shuffled); setCurrentIdx(0); setSelected(null); setScore(0); setAnswered(0); setShowResult(false); };

  if (questions.length === 0) return <p className="text-sm text-navy-400 py-8 text-center">{t('quiz.notEnough')}</p>;
    ...
    return (
      <div className="rounded-xl border border-cream-200 bg-white p-6 text-center space-y-4">
        <div className={`rounded-xl p-5 text-white ${pct >= 70 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-blue-500 to-indigo-600'}`}>
          ...
          <p className="text-sm opacity-80 mt-1">{pct}% · {t('quiz.estimatedLevel', { nclc })}</p>
        ...
        <button onClick={restart} className="px-5 py-2.5 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 inline-flex items-center gap-2"><Shuffle className="w-4 h-4" />{t('quiz.newQuiz')}</button>
      </div>
    );
  }

  const typeLabels: Record<string, string> = { synonym: t('quiz.synonym'), definition: t('quiz.definition'), fill_blank: t('quiz.fillBlank'), register: t('quiz.register') };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 text-xs text-navy-400">
        ...
        <div className="flex-1 bg-cream-100 rounded-full h-2 overflow-hidden"><div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${((currentIdx + (selected !== null ? 1 : 0)) / questions.length) * 100}%` }} /></div>
      ...
      <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-4">
        <div className="flex items-center gap-2"><span className={`px-2 py-0.5 rounded text-[10px] font-bold ${typeColors[q.type] || 'bg-gray-100'}`}>{typeLabels[q.type]}</span></div>
        ...
        <div className="space-y-2">
          {q.choices.map((choice, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleSelect(idx)} disabled={selected !== null} className={`w-full text-left px-4 py-3 rounded-lg border text-sm font-medium transition-all ${style}`}>
                <span className="font-mono text-[11px] mr-2 text-navy-300">{String.fromCharCode(65 + idx)}</span>{choice}
        ...
        {selected !== null && (
          <button onClick={nextQuestion} className="w-full py-2.5 rounded-lg bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 flex items-center justify-center gap-2">
            {currentIdx < questions.length - 1 ? <><ArrowRight className="w-4 h-4" />{t('quiz.nextQuestion')}</> : <><Trophy className="w-4 h-4" />{t('quiz.seeResult')}</>}
          </button>
        )}
      </div>
    </div>
...
function LexiqueDictee({ domain }: { domain: LexiqueDomain }) {
  ...
  useEffect(() => { const shuffled = [...domain.entries].sort(() => Math.random() - 0.5).slice(0, DICTEE_SIZE); setEntries(shuffled); setCurrentIdx(0); setUserInput(''); setChecked(false); setScore(0); setTotal(0); setShowResult(false); setHasInteracted(false); }, [domain]);

  const entry = entries[currentIdx];

  useEffect(() => {
    if (entry && !checked && hasInteracted) { const tm = setTimeout(() => tts.speak(entry.term, `dict-${currentIdx}`, 0.9, accent), 600); return () => clearTimeout(tm); }
  ...
  if (!entry || showResult) {
    ...
    return (
      <div className="rounded-xl border border-cream-200 bg-white p-6 text-center space-y-4">
        ...
        <h3 className="text-lg font-bold text-navy-900">{t('dict.done')}</h3>
        ...
        <p className="text-xs text-navy-400">+{score * TEF_XP_CONFIG.qcm_correct} XP</p>
        <button onClick={() => { const shuffled = [...domain.entries].sort(() => Math.random() - 0.5).slice(0, DICTEE_SIZE); setEntries(shuffled); setCurrentIdx(0); setUserInput(''); setChecked(false); setScore(0); setTotal(0); setShowResult(false); }} className="px-5 py-2.5 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 inline-flex items-center gap-2">
          <Shuffle className="w-4 h-4" />{t('dict.newDictee')}
        </button>
      </div>
    );
  }

  const normalize = (s: string) => s.toLowerCase().trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/['']/g, "'").replace(/\s+/g, ' ');
  const checkAnswer = () => { const normalizedUser = normalize(userInput); const normalizedTerm = normalize(entry.term); const correct = normalizedUser === normalizedTerm || userInput.toLowerCase().trim() === entry.term.toLowerCase().trim(); setIsCorrect(correct); setChecked(true); setTotal(t => t + 1); if (correct) { setScore(s => s + 1); addXp(TEF_XP_CONFIG.qcm_correct, 'TEF Lexique dictée'); } };
  const nextWord = () => { if (currentIdx < entries.length - 1) { setCurrentIdx(currentIdx + 1); setUserInput(''); setChecked(false); setTimeout(() => inputRef.current?.focus(), 100); } else setShowResult(true); };

  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 text-xs text-navy-400">
        <span>{currentIdx + 1}/{entries.length}</span>
        <div className="flex-1 bg-cream-100 rounded-full h-2 overflow-hidden"><div className="h-full bg-blue-400 rounded-full transition-all" style={{ width: `${((currentIdx + (checked ? 1 : 0)) / entries.length) * 100}%` }} /></div>
        <span className="text-emerald-600 font-bold">{score} ✓</span>
      </div>
      <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-4">
        <p className="text-sm text-navy-500 text-center">
          {!hasInteracted ? <span dangerouslySetInnerHTML={{ __html: t('dict.clickToStart') }} /> : t('dict.listenAndWrite')}
        </p>
        ...
        <div className="flex justify-center gap-3">
          <button onClick={() => playAudio(entry.term, `dict-${currentIdx}`, 0.8)} className={`px-4 py-2.5 rounded-lg text-sm font-medium inline-flex items-center gap-2 ${tts.speakingId === `dict-${currentIdx}` ? 'bg-blue-600 text-white' : 'bg-blue-50 text-blue-700 border border-blue-200 hover:bg-blue-100'}`}>
            <Volume2 className="w-4 h-4" />{t('lex.listen')}
          </button>
          <button onClick={() => playAudio(entry.term, `dict-slow-${currentIdx}`, 0.55)} className={`px-4 py-2.5 rounded-lg text-sm font-medium inline-flex items-center gap-2 ${tts.speakingId === `dict-slow-${currentIdx}` ? 'bg-blue-600 text-white' : 'bg-cream-50 text-navy-600 border border-cream-200 hover:bg-cream-100'}`}>
            <Volume2 className="w-4 h-4" />{t('dict.slow')}
          </button>
        </div>
        <div className="relative">
          <input ref={inputRef} type="text" value={userInput} onChange={e => setUserInput(e.target.value)}
            ...
            disabled={checked} placeholder={t('dict.placeholder')}
        ...
        {!checked ? (
          <button onClick={checkAnswer} disabled={!userInput.trim()} className="w-full py-2.5 rounded-lg bg-blue-600 text-white font-semibold text-sm hover:bg-blue-500 disabled:opacity-50 flex items-center justify-center gap-2">
            <CheckCircle2 className="w-4 h-4" />{t('dict.verify')}
          </button>
        ) : (
          <div className="space-y-3">
            {isCorrect ? (
              <div className="flex items-center gap-2 text-emerald-700 bg-emerald-50 rounded-lg px-4 py-2">
                <CheckCircle2 className="w-5 h-5" /><span className="text-sm font-bold">{t('dict.correct')}</span>
              ...
              <div className="bg-red-50 rounded-lg px-4 py-2 space-y-1">
                <div className="flex items-center gap-2 text-red-700"><XCircle className="w-5 h-5" /><span className="text-sm font-bold">{t('dict.incorrect')}</span></div>
                <p className="text-sm text-red-600">{t('dict.answer')} <strong>{entry.term}</strong></p>
              </div>
            )}
            <p className="text-xs text-navy-400"><em>{entry.definition}</em></p>
            <button onClick={nextWord} className="w-full py-2.5 rounded-lg bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 flex items-center justify-center gap-2">
              {currentIdx < entries.length - 1 ? <><ArrowRight className="w-4 h-4" />{t('dict.next')}</> : <><Trophy className="w-4 h-4" />{t('dict.result')}</>}
            </button>
          </div>
        )}
      </div>
...
function PiegesPanel() {
  ...
  const { t, tTrapTitle, tTrapName, tExp } = useTefLocale();
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('trap.foundation')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('trap.title')}</h2><p className="text-sm text-navy-400 max-w-2xl">{t('trap.subtitle')}</p></div>
      <div className="space-y-3">
            ...
            {openSections.has(idx) && (
              <div className="px-4 pb-4 border-t border-cream-100 space-y-3 pt-3">
                {section.traps.map((trap, ti) => (
                  <div key={ti} className="flex gap-3"><div className="shrink-0 w-1 rounded-full bg-red-200" /><div><p className="text-sm font-bold text-red-700 mb-0.5">{tTrapName(trap.name)}</p><p className="text-sm text-navy-500 leading-relaxed">{tExp(trap.description)}</p></div></div>
                ))}
              </div>
            )}
          </div>
...
function GamifiedQCMSession({ exercises, section, title, description, withTTS = false }: {
  ...
  if (phase === 'intro') {
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('qcm.section')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{title}</h2><p className="text-sm text-navy-400 max-w-2xl">{description}</p></div>
        <div className={`rounded-xl border ${config.borderColor} ${config.bgColor} p-6 text-center`}>
          <div className="text-4xl mb-3">{config.icon}</div>
          <h3 className="text-lg font-bold text-navy-900 mb-1">{t(`section.${section}`)}</h3>
          <div className="flex justify-center gap-6 mt-4 text-sm">
            <div><span className="text-2xl font-bold text-navy-900">{exercises.length}</span><p className="text-navy-400 text-xs">{t('qcm.exercises')}</p></div>
            <div><span className="text-2xl font-bold text-navy-900">{totalQuestions}</span><p className="text-navy-400 text-xs">{t('qcm.questionsLabel')}</p></div>
            <div><span className="text-2xl font-bold text-teal-600">{exercises.reduce((s, e) => s + e.points * e.questions.length, 0)}</span><p className="text-navy-400 text-xs">{t('qcm.xpMax')}</p></div>
          </div>
          {withTTS && (
            <div className="mt-4 space-y-2">
              <p className="text-xs text-navy-400 flex items-center justify-center gap-1"><Volume2 className="w-3.5 h-3.5" /> {t('qcm.audioTTS')}</p>
              <div className="flex justify-center gap-2">
                <button onClick={() => setAccent('france')} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${accent === 'france' ? 'bg-blue-100 text-blue-700 border border-blue-300' : 'bg-cream-50 text-navy-500 border border-cream-200 hover:bg-cream-100'}`}>🇫🇷 France</button>
                <button onClick={() => setAccent('quebec')} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${accent === 'quebec' ? 'bg-blue-100 text-blue-700 border border-blue-300' : 'bg-cream-50 text-navy-500 border border-cream-200 hover:bg-cream-100'}`}>🇨🇦 Québec</button>
              </div>
          ...
          <button onClick={start} className="mt-6 px-8 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 transition-colors inline-flex items-center gap-2">
            <Zap className="w-4 h-4" /> {t('qcm.startTraining')}
          </button>
        </div>
      </div>
    );
  ...
  if (phase === 'results') {
    ...
    const totalA = answers.length;
    const pct = totalA > 0 ? Math.round((correct / totalA) * 100) : 0;
    ...
    trapsTriggered.forEach(tp => trapCounts.set(tp, (trapCounts.get(tp) || 0) + 1));
    const elapsedEnd = Math.round((Date.now() - startedAt) / 1000);
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        <div className={`rounded-xl overflow-hidden ${pct >= 60 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-orange-400 to-red-500'} px-6 py-10 text-white text-center`}>
          ...
          <p className="text-5xl font-extrabold">{correct}<span className="text-2xl opacity-70">/{totalA}</span></p>
          <p className="text-xl font-semibold mt-2">{t('qcm.nclcEstimated', { nclc })}</p>
          <p className="text-sm opacity-80 mt-1">{pct}% · {Math.floor(elapsedEnd / 60)}:{(elapsedEnd % 60).toString().padStart(2, '0')}</p>
        ...
        {trapCounts.size > 0 && (
          <div className="rounded-xl border border-red-200 bg-red-50 p-5">
            <p className="text-xs font-semibold text-red-600 uppercase tracking-wider mb-3">{t('qcm.trapsTriggered')}</p>
        ...
        <button onClick={restart} className="w-full px-6 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 transition-colors flex items-center justify-center gap-2">
          <RotateCcw className="w-4 h-4" /> {t('qcm.restart')}
        </button>
      </div>
    );
  }
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="flex items-center gap-3">
        ...
        <div className={`flex items-center gap-1 px-2 py-1 rounded-full border ${elapsed > timeLimit * 0.8 ? 'bg-red-50 border-red-200' : 'bg-cream-50 border-cream-200'}`}>
          ...
          <span className={`text-xs font-bold font-mono ${elapsed > timeLimit * 0.8 ? 'text-red-600' : 'text-navy-500'}`}>{Math.floor(elapsed / 60)}:{(elapsed % 60).toString().padStart(2, '0')}</span>
        </div>
      ...
      <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-4">
          ...
          <div className="p-3 rounded-lg bg-blue-50 border border-blue-200 space-y-2">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-blue-800">{t('qcm.listenPassage')} {accent === 'france' ? '🇫🇷' : '🇨🇦'}</p>
                <p className="text-xs text-blue-600">{t('qcm.clickToListen')} {current.ttsSpeed}x</p>
              </div>
        ...
        {withTTS && current.stimulus && allCurrentAnswered && (
          <div>
            ...
            {showTranscript && (<div className="mt-2 bg-cream-50 border-l-[3px] border-blue-300 px-4 py-3 text-sm text-navy-700 leading-relaxed italic whitespace-pre-line">{current.stimulus}</div>)}
          </div>
        )}
        ...
        {allCurrentAnswered && (
          <button onClick={() => { setShowTranscript(false); nextExercise(); }} className="w-full px-6 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 transition-colors flex items-center justify-center gap-2">
            {currentIdx + 1 >= exercises.length ? t('qcm.seeResults') : t('qcm.nextExercise')}
...
function EETrainingPanel() {
  ...
  const grilleData = locale === 'en' ? eeGrilleEN : eeGrille;

  useEffect(() => {
    if (selectedExercise !== null && !analysis && eeStartedAt > 0) { const tm = setInterval(() => setEeElapsed(Math.round((Date.now() - eeStartedAt) / 1000)), 1000); return () => clearInterval(tm); }
  ...
  const submitText = async () => {
    ...
    setSessionXP(xp); addXp(xp, 'TEF EE exercise');
  ...
  if (selectedExercise === null) {
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('ee.sectionLabel')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('ee.title')}</h2><p className="text-sm text-navy-400 max-w-2xl">{t('ee.description')}</p></div>
        <div className="rounded-xl border border-cream-200 bg-white p-5">
          <h3 className="text-base font-semibold text-navy-900 mb-3 flex items-center gap-2"><Award className="w-4 h-4 text-emerald-600" />{t('ee.gradingGrid')}</h3>
          <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b-2 border-navy-100">{grilleData.headers.map((h, i) => <th key={i} className="text-left py-2 px-3 text-[11px] font-semibold tracking-wider uppercase text-navy-400">{h}</th>)}</tr></thead><tbody className="divide-y divide-cream-100">{grilleData.rows.map((row, ri) => <tr key={ri}>{row.map((cell, ci) => <td key={ci} className={`py-2 px-3 ${ci === 0 ? 'font-medium text-navy-800' : 'text-navy-500'} text-sm`}>{cell}</td>)}</tr>)}</tbody></table></div>
        </div>
        {eeExercises.map((ex, idx) => (
          <button key={idx} onClick={() => { setSelectedExercise(idx); setEeStartedAt(Date.now()); setEeElapsed(0); }} className="w-full rounded-xl border border-cream-200 bg-white p-5 text-left hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center gap-2 mt-3 text-xs text-navy-400"><PenTool className="w-3.5 h-3.5" />{ex.minWords}-{ex.maxWords} {t('ee.words')} · {t('ee.criteria5')} · +{TEF_XP_CONFIG.writing_submit} XP</div>
          </button>
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300">
      <button onClick={reset} className="text-sm text-navy-400 hover:text-navy-700 flex items-center gap-1"><ChevronDown className="w-4 h-4 rotate-90" /> {t('ee.backToSubjects')}</button>
      <div className="rounded-xl border border-cream-200 bg-white p-5">
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-medium text-navy-400">{exercise!.meta}</p>
          {!analysis && (<div className={`flex items-center gap-1 px-2 py-1 rounded-full border ${eeElapsed > 3000 ? 'bg-red-50 border-red-200' : 'bg-cream-50 border-cream-200'}`}><Clock className={`w-3 h-3 ${eeElapsed > 3000 ? 'text-red-500' : 'text-navy-400'}`} /><span className={`text-xs font-bold font-mono ${eeElapsed > 3000 ? 'text-red-600' : 'text-navy-500'}`}>{Math.floor(eeElapsed / 60)}:{(eeElapsed % 60).toString().padStart(2, '0')}</span></div>)}
        </div>
        <h3 className="text-sm font-semibold text-navy-800 mb-4">{exercise!.sujet}</h3>
        <textarea value={userText} onChange={e => setUserText(e.target.value)} placeholder={t('ee.placeholder')} className="w-full min-h-[200px] p-4 rounded-lg border border-cream-200 text-sm text-navy-700 focus:ring-2 focus:ring-blue-300 focus:border-blue-400 outline-none resize-y" disabled={!!analysis} />
        <div className="flex items-center justify-between mt-3">
          <span className={`text-xs font-medium ${userText.split(/\s+/).filter(w => w).length < exercise!.minWords ? 'text-red-500' : 'text-emerald-600'}`}>
            {userText.split(/\s+/).filter(w => w).length} / {exercise!.minWords} {t('ee.wordsMin')}
          </span>
          {!analysis && <button onClick={submitText} disabled={userText.split(/\s+/).filter(w => w).length < 10} className="px-5 py-2 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"><Send className="w-4 h-4" />{t('ee.submit')}</button>}
        </div>
      </div>
      ...
      {analysis && (
        <div className="space-y-4">
          {(() => {
            ...
            const totalScore = aiEval.writingEval?.totalScore ?? Object.values(analysis.criteriaScores).reduce((a, b) => a + b, 0);
            return (
              <div className={`rounded-xl p-6 text-center text-white ${nclc >= 7 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-blue-500 to-indigo-600'}`}>
                ...
                <p className="text-sm opacity-80 mt-1">{analysis.wordCount} {t('ee.words')} · {totalScore}/100 pts · {t('ee.connectors')} {analysis.connectorsLevel}</p>
          ...
          {aiEval.isEvaluating && (<div className="rounded-xl border border-blue-200 bg-blue-50 p-4 flex items-center gap-3"><div className="w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" /><p className="text-sm text-blue-700 font-medium">{t('ee.aiEvaluating')}</p></div>)}

          {aiEval.writingEval && (
            <div className="space-y-4">
              <div className="rounded-xl border border-indigo-200 bg-white p-5 space-y-3">
                <h4 className="text-sm font-semibold text-navy-800 flex items-center gap-2"><Award className="w-4 h-4 text-indigo-600" />{t('ee.aiCriteria')}</h4>
                {[
                  { key: 'respectTache', label: t('ee.respectTask') },
                  { key: 'organisation', label: t('ee.organisation') },
                  { key: 'richesseLexicale', label: t('ee.lexicalRichness') },
                  { key: 'grammaire', label: t('ee.grammar') },
                  { key: 'cinquiemeCritere', label: selectedExercise === 0 ? t('ee.narrativeQuality') : t('ee.argumentativeQuality') },
                ].map(({ key, label }) => {
              ...
              <div className="rounded-xl border border-cream-200 bg-cream-50 p-5 space-y-3">
                ...
                {aiEval.writingEval.strengths.length > 0 && (<div><p className="text-xs font-semibold text-emerald-700 mb-1">{t('ee.strengths')}</p><ul className="space-y-1">{aiEval.writingEval.strengths.map((s, i) => <li key={i} className="text-sm text-navy-600 flex gap-2"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />{s}</li>)}</ul></div>)}
                {aiEval.writingEval.improvements.length > 0 && (<div><p className="text-xs font-semibold text-amber-700 mb-1">{t('ee.improvements')}</p><ul className="space-y-1">{aiEval.writingEval.improvements.map((s, i) => <li key={i} className="text-sm text-navy-600 flex gap-2"><AlertTriangle className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />{s}</li>)}</ul></div>)}
              </div>

              {aiEval.writingEval.correctedExcerpts.length > 0 && (
                <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-3">
                  <h4 className="text-sm font-semibold text-navy-800">{t('ee.suggestedCorrections')}</h4>
                  {aiEval.writingEval.correctedExcerpts.map((c, i) => (<div key={i} className="bg-cream-50 rounded-lg p-3 space-y-1 border border-cream-100"><p className="text-xs text-red-600 line-through">{c.original}</p><p className="text-xs text-emerald-700 font-medium">{c.corrected}</p><p className="text-xs text-navy-500 italic">{c.explanation}</p></div>))}
                </div>
              )}
            </div>
          )}
          ...
          {aiEval.evalError && (
            <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-3">
              <h4 className="text-sm font-semibold text-navy-800">{t('ee.localEval')}</h4>
              {exercise!.criteria.map((c, i) => {
                const sc = Object.values(analysis.criteriaScores)[i] ?? 0;
                const pct = Math.round((sc / c.maxPoints) * 100);
                return (<div key={c.id} className="space-y-1"><div className="flex justify-between text-xs"><span className="font-medium text-navy-700">{c.label}</span><span className={`font-bold ${pct >= 70 ? 'text-emerald-600' : pct >= 50 ? 'text-amber-600' : 'text-red-500'}`}>{sc}/{c.maxPoints}</span></div><div className="bg-cream-100 rounded-full h-2 overflow-hidden"><div className={`h-full rounded-full transition-all ${pct >= 70 ? 'bg-emerald-400' : pct >= 50 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${pct}%` }} /></div></div>);
              })}
            </div>
          )}

          {analysis.connectorsUsed.length > 0 && (<div className="rounded-xl border border-cream-200 bg-white p-4"><p className="text-xs font-semibold text-navy-500 mb-2">{t('ee.connectorsDetected')}</p><div className="flex flex-wrap gap-1.5">{analysis.connectorsUsed.map((c, i) => <span key={i} className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-xs font-medium border border-blue-200">{c}</span>)}</div></div>)}
          ...
          <button onClick={() => setShowModel(!showModel)} className="w-full flex items-center justify-between px-4 py-3 rounded-xl border border-cream-200 bg-white text-sm font-medium text-navy-700 hover:bg-cream-50">
            <span className="flex items-center gap-2">{showModel ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}{t('ee.showModels')}</span>
            <ChevronDown className={`w-4 h-4 transition-transform ${showModel ? 'rotate-180' : ''}`} />
          </button>
          {showModel && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-cream-50 rounded-lg p-4 border border-cream-200"><span className="inline-block font-mono text-[11px] font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded mb-2">{t('ee.modelN6')}</span><p className="text-sm text-navy-600 leading-relaxed">{exercise!.modelTexts.nclc6}</p></div>
              <div className="bg-cream-50 rounded-lg p-4 border border-cream-200"><span className="inline-block font-mono text-[11px] font-bold text-indigo-700 bg-indigo-100 px-2 py-0.5 rounded mb-2">{t('ee.modelN9')}</span><p className="text-sm text-navy-600 leading-relaxed">{exercise!.modelTexts.nclc9}</p></div>
            </div>
          )}
          <button onClick={reset} className="w-full px-6 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 flex items-center justify-center gap-2"><RotateCcw className="w-4 h-4" />{t('ee.newExercise')}</button>
        </div>
      )}
    </div>
  );
...
function EOTrainingPanel() {
  ...
  const startRecording = () => { sr.resetTranscript(); sr.startListening(); setRecordingStart(Date.now()); setPhase('speak'); };
  const stopAndAnalyze = () => { sr.stopListening(); setPendingAnalysis(true); };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setSessionXP(xp); addXp(xp, 'TEF EO exercise'); setPhase('results'); setPendingAnalysis(false);
      if (exercise) {
        const variant = exercise.variants[selectedVariant];
        aiEval.evaluateSpeaking(transcriptRef.current, exercise.scenario, duration, { type: variant.type, examinerLine: variant.examinerLine, expectedSkill: variant.expectedSkill });
      }
    }, 500);
  ...
  if (selectedExercise === null) {
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('eo.sectionLabel')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('eo.title')}</h2><p className="text-sm text-navy-400 max-w-2xl">{t('eo.description')}</p></div>
        {!sr.isSupported && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 flex items-start gap-3">
            <AlertTriangle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div><p className="text-sm font-medium text-amber-800">{t('eo.srNotAvailable')}</p><p className="text-xs text-amber-600 mt-1">{t('eo.srNotSupported')}</p></div>
          </div>
        )}
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300">
      <button onClick={reset} className="text-sm text-navy-400 hover:text-navy-700 flex items-center gap-1"><ChevronDown className="w-4 h-4 rotate-90" /> {t('eo.backToScenarios')}</button>
      ...
      <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <p className="text-xs font-semibold text-amber-700 mb-2">{t('eo.examinerSays')}</p>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => tts.speak(variant.examinerLine.replace(/[«»]/g, ''), `eo-${selectedVariant}`, exercise!.ttsSpeed ?? 1.0, accent)} className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tts.speakingId === `eo-${selectedVariant}` ? 'bg-amber-600 text-white' : 'bg-white text-amber-700 border border-amber-300 hover:bg-amber-100'}`}>
              ...
              {t('eo.listenAccent')} {accent === 'france' ? '🇫🇷' : '🇨🇦'}
            </button>
            {tts.isSpeaking && <span className="text-xs text-amber-600 animate-pulse">{t('qcm.playbackInProgress')}</span>}
          </div>
          <p className="text-xs text-amber-600 mt-2">{t('eo.skillTested')} {variant.expectedSkill}</p>
        </div>

        {phase === 'listen' && (
          <div className="text-center py-4">
            <p className="text-sm text-navy-500 mb-4">{t('eo.listenThenRecord')}</p>
            <button onClick={startRecording} disabled={!sr.isSupported} className="px-6 py-3 rounded-xl bg-emerald-600 text-white font-semibold text-sm hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2">
              <Mic className="w-5 h-5" /> {t('eo.startSpeaking')}
            </button>
          </div>
        )}
        ...
        {phase === 'speak' && (
          <div className="text-center py-4 space-y-4">
            <div className="inline-flex items-center gap-3 px-4 py-3 rounded-xl bg-red-50 border border-red-200">
              ...
              <span className="text-sm font-medium text-red-700">{t('eo.recording')}</span>
            </div>
            ...
            <button onClick={stopAndAnalyze} className="px-6 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 inline-flex items-center gap-2">
              <Square className="w-4 h-4" /> {t('eo.stopAndAnalyze')}
            </button>
          </div>
        )}
        ...
        {phase === 'results' && speechResult && (
          <div className="space-y-4">
            {(() => {
              ...
              return (
                <div className={`rounded-xl p-5 text-center text-white ${nclc >= 7 ? 'bg-gradient-to-br from-emerald-500 to-teal-600' : 'bg-gradient-to-br from-blue-500 to-indigo-600'}`}>
                  ...
                  <p className="text-sm opacity-80 mt-1">{speechResult.wordCount} {t('ee.words')} · {speechResult.wordsPerMinute} {t('eo.wordsPerMin')} · {t('eo.confidence')} {Math.round(speechResult.confidence * 100)}%</p>
            ...
            {sr.transcript && (<div className="rounded-xl border border-cream-200 bg-cream-50 p-4"><p className="text-xs font-semibold text-navy-500 mb-2">{t('eo.yourTranscript')}</p><p className="text-sm text-navy-700">{sr.transcript}</p></div>)}

            {aiEval.isEvaluating && (<div className="rounded-xl border border-blue-200 bg-blue-50 p-4 flex items-center gap-3"><div className="w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin" /><p className="text-sm text-blue-700 font-medium">{t('eo.aiEvaluating')}</p></div>)}

            {aiEval.speakingEval && (
              <div className="space-y-4">
                <div className="rounded-xl border border-indigo-200 bg-white p-5 space-y-3">
                  <h4 className="text-sm font-semibold text-navy-800 flex items-center gap-2"><Award className="w-4 h-4 text-indigo-600" />{t('eo.aiCriteria')}</h4>
                  {[
                    { key: 'fluidite', label: t('eo.fluidity') },
                    { key: 'richesseLexicale', label: t('eo.vocabRichness') },
                    { key: 'interaction', label: t('eo.interaction') },
                    { key: 'connecteurs', label: t('eo.connectors') },
                    { key: 'registre', label: t('eo.registerLabel') },
                  ].map(({ key, label }) => {
                ...
                <div className="rounded-xl border border-cream-200 bg-cream-50 p-5 space-y-3">
                  ...
                  {aiEval.speakingEval.strengths.length > 0 && (<div><p className="text-xs font-semibold text-emerald-700 mb-1">{t('ee.strengths')}</p><ul className="space-y-1">{aiEval.speakingEval.strengths.map((s, i) => <li key={i} className="text-sm text-navy-600 flex gap-2"><CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />{s}</li>)}</ul></div>)}
                  {aiEval.speakingEval.improvements.length > 0 && (<div><p className="text-xs font-semibold text-amber-700 mb-1">{t('ee.improvements')}</p><ul className="space-y-1">{aiEval.speakingEval.improvements.map((s, i) => <li key={i} className="text-sm text-navy-600 flex gap-2"><AlertTriangle className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />{s}</li>)}</ul></div>)}
                </div>

                {aiEval.speakingEval.suggestedRephrasing.length > 0 && (
                  <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-3">
                    <h4 className="text-sm font-semibold text-navy-800">{t('eo.suggestedRephrasing')}</h4>
                    {aiEval.speakingEval.suggestedRephrasing.map((r, i) => (<div key={i} className="bg-cream-50 rounded-lg p-3 space-y-1 border border-cream-100"><p className="text-xs text-navy-500">{r.original}</p><p className="text-xs text-emerald-700 font-medium">→ {r.improved}</p><p className="text-xs text-navy-400 italic">{r.why}</p></div>))}
                  </div>
                )}
              </div>
            )}
            ...
            {!aiEval.speakingEval && !aiEval.isEvaluating && (
              <div className="rounded-xl border border-cream-200 bg-white p-5 space-y-3">
                <h4 className="text-sm font-semibold text-navy-800">{t('eo.localScores')}</h4>
                {[
                  { label: t('eo.fluencyLabel'), score: speechResult.fluencyScore, max: 20 },
                  { label: t('eo.contentLabel'), score: speechResult.contentScore, max: 20 },
                  { label: t('eo.interactionLabel'), score: speechResult.interactionScore, max: 20 },
                ].map(({ label, score, max }) => {
                  const pct = Math.round((score / max) * 100);
                  return (<div key={label} className="space-y-1"><div className="flex justify-between text-xs"><span className="font-medium text-navy-700">{label}</span><span className={`font-bold ${pct >= 70 ? 'text-emerald-600' : pct >= 50 ? 'text-amber-600' : 'text-red-500'}`}>{score}/{max}</span></div><div className="bg-cream-100 rounded-full h-2 overflow-hidden"><div className={`h-full rounded-full ${pct >= 70 ? 'bg-emerald-400' : pct >= 50 ? 'bg-amber-400' : 'bg-red-400'}`} style={{ width: `${pct}%` }} /></div></div>);
                })}
              </div>
            )}

            <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
              <p className="text-xs font-semibold text-blue-700 mb-2">{t('eo.tipsNCLC7')}</p>
...
function DiagnosticPanel() {
  ...
  const handleAnswer = (qIdx: number, choiceIdx: number) => { if (answers[qIdx] !== undefined) return; setAnswers(prev => ({ ...prev, [qIdx]: choiceIdx })); };
  const score = useMemo(() => Object.entries(answers).reduce((acc, [qIdx, chosen]) => acc + (diagnosticExercises[Number(qIdx)].questions[0].correctIndex === chosen ? 1 : 0), 0), [answers]);
  const allAnswered = Object.keys(answers).length === diagnosticExercises.length;
  const submitResult = () => { setShowResult(true); addXp(score * TEF_XP_CONFIG.qcm_correct + TEF_XP_CONFIG.session_complete, 'TEF Diagnostic'); };

  const getResult = () => {
    if (score <= 3) return { stade: t('diag.stageI'), action: t('diag.stageI_action'), color: 'bg-orange-50 border-orange-200 text-orange-800', nclc: 4 };
    if (score <= 5) return { stade: t('diag.stageIIlow'), action: t('diag.stageIIlow_action'), color: 'bg-blue-50 border-blue-200 text-blue-800', nclc: 6 };
    if (score <= 7) return { stade: t('diag.stageIIhigh'), action: t('diag.stageIIhigh_action'), color: 'bg-emerald-50 border-emerald-200 text-emerald-800', nclc: 7 };
    return { stade: t('diag.stageIII'), action: t('diag.stageIII_action'), color: 'bg-indigo-50 border-indigo-200 text-indigo-800', nclc: 9 };
  };

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('diag.trainingLabel')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('diag.title')}</h2><p className="text-sm text-navy-400 max-w-2xl">{t('diag.subtitle')}</p></div>
      <div className="space-y-4">
      ...
      {allAnswered && !showResult && (<button onClick={submitResult} className="w-full px-6 py-3 rounded-xl bg-navy-800 text-white font-semibold text-sm hover:bg-navy-700 flex items-center justify-center gap-2"><Target className="w-4 h-4" />{t('diag.seeResult')}</button>)}
      {showResult && (() => {
        const result = getResult();
        return (
          <div className={`rounded-xl border p-6 ${result.color} space-y-2`}>
            <div className="flex items-center justify-between"><p className="text-lg font-bold">{t('diag.result')} {score}/{diagnosticExercises.length}</p><span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/60 text-sm font-bold"><Zap className="h-3.5 w-3.5" />+{score * TEF_XP_CONFIG.qcm_correct + TEF_XP_CONFIG.session_complete} XP</span></div>
            <p><strong>{t('diag.stageEstimated')}</strong> {result.stade}</p>
            <p><strong>{t('diag.recommendation')}</strong> {result.action}</p>
          </div>
        );
      })()}
    </div>
...
function BaremePanel() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div><p className="text-[11px] font-semibold tracking-wider uppercase text-navy-300 mb-1">{t('bareme.validation')}</p><h2 className="text-xl font-bold text-navy-900 mb-2">{t('bareme.title')}</h2><p className="text-sm text-navy-400 max-w-2xl">{t('bareme.subtitle')}</p></div>
